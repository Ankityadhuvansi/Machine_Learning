...
def train_test_split(X, y, ratio=0.8):
    ...
    # A slice with first train_len elements of X will be the training sample for objects.
    X_train = X[indices[:train_len]]
    # A slice with the rest of X will be the testing sample for objects.
    X_test = X[indices[train_len:]]

    # A slice with first train_len of y will be the training sample for classes.
    y_train = y[indices[:train_len]]
    # A slice with the rest of y will be the testing sample for classes.
...
if __name__ == '__main__':
    ...
    wines = np.genfromtxt('wine.csv', delimiter=',', skip_header=1)
